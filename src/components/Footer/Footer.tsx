...
const App = ({ footer }: FooterProps) => {
  const section = footer.sections[0];
  if (!section) {
    return (
      <footer className="py-12 w-full font-sans text-black">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 text-center text-gray-500">
...
console.log(section)
  return (
    <footer className=" py-12 w-full font-sans text-black">
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 grid grid-cols-1 md:grid-cols-4 gap-8">
        {/* Column 1: Product Categories */}
        <div className="flex flex-col space-y-2">
          {(section.category ?? []).map((category, index) => (
            <a key={index} href={category.url} className="hover:text-gray-900 text-[#5B4B43] font-thin text-sm transition-colors duration-200">
              {category.label}
            </a>
          ))}
        </div>
        ...
        {/* Column 2: Seasons */}
        <div className="flex flex-col space-y-2">
          {(section.season ?? []).map((season, index) => (
            <a  key={index} href={season.url} className="hover:text-gray-900 text-[#5B4B43] font-thin text-sm transition-colors duration-200">
              {season.label}
            </a>
          ))}
        </div>
        ...
        <div className="flex flex-col space-y-2">
          {(section.about ?? []).map((link, index) => (
            <a key={index} href={link.url} className="hover:text-gray-900 text-sm font-thin text-[#5B4B43] transition-colors duration-200">
              {link.label}
            </a>
          ))}
        </div>
      ...
      {/* Payment Methods */}
      <div className="flex justify-center space-x-3 mt-10 mb-8 max-w-7xl mx-auto px-4 md:px-8 lg:px-16">
